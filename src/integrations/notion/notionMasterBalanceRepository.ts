...
export function createNotionMasterBalanceRepository(options: Options): MasterBalanceRepository {
  ...
  return {
    ...
    async findLatestAccepted() {
      ...
      const baseCurrency = selectProperty(props["Основная валюта"]);
      if (!(CURRENCIES as readonly string[]).includes(baseCurrency)) throw new Error("Notion balance anchor currency is invalid.");
      return {
        ...
        balance: numberProperty(props["Итоговый остаток"]),
...
function properties(input: MasterBalanceObservationWrite) {
  return {
    "Наблюдение": { title: [{ type: "text", text: { content: input.account ? `Остаток: ${input.account}` : "Общий остаток" } }] },
    "Дата": { date: { start: input.occurredOn } },
    "Исходная сумма": { number: input.originalAmount },
    "Валюта": { select: { name: input.originalCurrency } },
    "Курс к основной валюте": { number: input.conversionRate },
    "Сумма в основной валюте": { number: input.baseAmount },
    "Основная валюта": { select: { name: input.baseCurrency } },
    "Счёт": input.account ? { select: { name: input.account } } : { select: null },
    "Рассчитанный остаток": { number: input.calculatedBalance },
    "Итоговый остаток": { number: input.acceptedBalance },
    "Якорь": { checkbox: input.isAnchor },
    "Разница": { number: input.difference },
    "Допуск": { number: input.tolerance },
...
function validate(input: MasterBalanceObservationWrite) {
  ...
  if (!(CURRENCIES as readonly string[]).includes(input.baseCurrency)) throw new Error("Balance base currency is not supported.");
  if (!Number.isFinite(input.conversionRate) || input.conversionRate <= 0) throw new Error("Balance conversion rate must be positive.");
  for (const value of [input.baseAmount, input.calculatedBalance, input.acceptedBalance, input.difference, input.tolerance]) {
  ...
  if (typeof input.isAnchor !== "boolean") throw new Error("Balance anchor flag is invalid.");
...
function selectProperty(value: unknown) { return requiredString(requireRecord(requireRecord(value, "select property").select, "select").name, "select name"); }
