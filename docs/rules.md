...
- Keep the parser cache key `budget-parse-toon-v5` and revision cache key `budget-revise-toon-v5` separate, with an explicit breakpoint after reusable developer instructions and before changing TOON data. Bump the key version when the reusable prompt contract changes materially.
