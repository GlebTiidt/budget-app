...
- Keep the parser cache key `budget-parse-toon-v3` and revision cache key `budget-revise-toon-v3` separate, with an explicit breakpoint after reusable developer instructions and before changing TOON data. Bump the key version when the reusable prompt contract changes materially.
- The current independently numbered debt-schema `v3` verification is 15 live requests: all 12 parser cases, both transaction reply revisions, and the `долг 1` debt revision passed with `reasoning.effort: none`, using 22,499 total tokens with 12,232 cached input tokens. The earlier `v1` reasoning comparison and `v2` debt baseline remain regression references, not universal savings. Full measurements live in [`docs/token-optimization.md`](token-optimization.md).
...
- A confirmed user-stated balance is an authoritative reconciliation observation. Compare it with the application-calculated balance using a documented tolerance that accounts for differences between actual exchange execution and the official historical rate. If the values agree within tolerance, accept the observation as the new balance anchor. If they differ beyond tolerance, show the difference and ask about missing operations before replacing the calculated anchor; if the user explicitly confirms that nothing is missing, accept and persist the observed balance without fabricating an adjustment transaction.
...
- Show all transaction and debt drafts from one user message in one Telegram preview and manage it through a normal text reply. Number ordinary operations and debt operations independently from `1` inside their separate sections, without a `Д` prefix. A plain correction number targets `Операции`; require `долг N` or `долговая операция N` to target the independently numbered debt section. Show a balance observation only in the summary.
...
- Chat cleanup happens only after every requested financial write succeeds idempotently. Never delete the source message or preview while a save is pending, failed, or only simulated in preview mode.
