...
- Record stable accounting and interaction decisions here; keep deployment IDs, dated verification results, and outstanding work in `docs/checklist.md`. A repaired incident does not imply that every planned feature or owner smoke test is complete.
- Before deploying functional changes, run `npm run typecheck`, `npm test`, and `npm run build`; changes to parsing prompts, schemas, limits, or revision behavior also require `npm run test:parser:live -- --compact`. Use synthetic regression cases for the reported failure, including expected amounts and dates rather than only row counts.
- When production deployment is requested, deploy to the existing Vercel project and verify `Ready`, the production alias, webhook health, and relevant authorization/behavior checks. Report deployment as complete only after verification. Intercepted Telegram transport checks must not be described as real owner confirmation tests.
...
- Use `text.verbosity: low`, a `24,000`-token response ceiling, and bounded Structured Output strings for the GPT-5.6 budget parser. Change these limits only after the maximum-size structured response and live parser suite still pass.
- Keep the parser cache key `budget-parse-toon-v8` and revision cache key `budget-revise-toon-v8` separate, with an explicit breakpoint after reusable developer instructions and before changing TOON data. Bump the key version when the reusable prompt contract changes materially.
...
- A preview correction sends the stored normalized draft, with transactions in visible preview order, and the user's direct correction to the language model. Never send the rendered calculated summary as financial evidence or include unrelated messages or raw history.
- The stored normalized draft, direct reply, controlled catalogs, timestamp, and timezone are serialized into one TOON input document for correction requests; rendered bot text is never a substitute for that draft.
...
- Preview income and expense describe complete non-transfer operations in the current draft only. The projected total balance is calculated from confirmed history plus that draft, excluding already persisted source IDs. Distinguish this projection from the confirmed current balance in answers and repair reports. Without a known anchor, never present one message's income minus expense as the user's real balance.
...
- Keep `/start`, `/settings`, `/reports`, `/help`, `/balance`, `/month`, and `/debts` in the Telegram command menu; support `/history` and ordinary budget questions as well.
...
- Show all transaction and debt drafts from one user message in one logical Telegram preview, split across complete HTML-safe messages when necessary, and manage it through native `Всё верно` and `Исправить` actions or a normal text reply. Hide the confirmation action while any field remains unresolved. Number ordinary operations and debt operations independently from `1` inside their separate sections, without a `Д` prefix. A plain correction number targets `Операции`; require `долг N` or `долговая операция N` to target the independently numbered debt section. Show a balance observation only in the summary.
...
- Never truncate a preview, clarification, comment, or ambiguity to fit Telegram's 4,096-character limit. Split oversized previews into complete HTML-safe messages without dropping content; preserve every numbered item, every missing-field request, the reply instructions, and the confirmation warning.
...
## Accounting and Conversation Invariants

These rules preserve the fixes established during the 2026-09-06 incident. Their verification and deployment history live in `docs/checklist.md`.

- Read the complete paginated confirmed operation history to calculate balances. Use transaction date first and Telegram order only to break same-day ties. The settings opening balance starts its effective day regardless of an imported observation order. Later accepted snapshots reconcile the chronological total.
- Never use a stored running balance as the input to the next calculation. Recalculate affected derived rows after saving, including backdated inserts and retries; current-balance answers compute from history without writes.
- Recognized balance, monthly report, category, debt, and history questions are read-only and bypass the transaction parser. They do not replace or confirm pending drafts.
- Answer ordinary questions such as `Какой у меня остаток?`, `Какие расходы в этом месяце?`, and `Расходы по категориям за прошлый месяц` from confirmed records. Use the operation date and the user's timezone for period selection, including backdated entries. Explicitly identify the reporting period and currency; do not include unconfirmed drafts in historical totals.
- Unthreaded explicit corrections and confirmations may resolve the latest unexpired draft scoped to both Telegram chat and owner. New financial statements create their own draft; explicit Telegram replies continue to target their selected draft.
- Conversational memory consists of persisted normalized drafts, controlled settings, and confirmed ledger history. It must survive Vercel cold starts and deployments; neither process memory nor a model's recollection is authoritative. Preserve the existing 24-hour draft expiry and report an expired or unavailable draft rather than reconstructing it from an unrelated message.
- A question quoting the bot's balance, such as `Откуда этот остаток?`, is not a new user-stated balance observation. Corrections must preserve unchanged operations and cannot turn a calculated summary into an accepted balance anchor.
- Send a successful receipt before deleting source or preview messages. Retain the draft when financial persistence fails.
- Until base-currency migration is implemented, do not let the master settings silently switch away from the opening/history currency. Explain that a full conversion is required rather than mixing currencies in reports.

- Long previews must never discard operations or truncate their tails. Keep global numbering across pages, a single final summary, and confirmation/correction actions on the final page; replies to any current page resolve the same draft.
- The parser currently supports up to 100 transactions and 100 debt operations with a 24,000-token output ceiling. Reject incomplete API responses and never accept a partial prefix of an oversized input as a complete draft. Process every dated section, inherit explicitly established currency context, retain each separately listed unknown expense, and combine an explicit sum such as `1700+2300` for one purchase without dropping either term. An ordinary correction cannot reduce operation counts unless the user requests removal or merging.
- Explicitly undefined expenses use the existing `Другое` category. A short `Нет категории` reply fills only missing expense categories without inventing a purpose or changing known categories.
- Calculate conversion per operation using its historical rate and the application's currency rounding, then sum those converted amounts consistently for previews, receipts, and reports. Share in-flight rate requests by date/currency pair without sharing converted amounts; evict failed requests so a retry can recover.
- Currency conversion during a transfer between the user's own accounts does not turn that transfer into an expense or reduce the calculated total. Never invent a fee or exchange loss to explain a discrepancy; any separately stated expense follows the normal confirmation flow.
- A draft restored after omitted rows must be shown in full before confirmation can persist it. `Покажи черновик` refreshes the current draft without new financial facts.
- Before repairing production balances or drafts, keep a private ignored backup, compare source operations with the supplied evidence, and check for already persisted rows. Repair derived values without rewriting original amounts, currencies, dates, or directions. Restoring a draft does not authorize saving its expenses; verify the repaired data by reading it back and state whether the reported balance is confirmed or projected.
- Keep the personal Vercel webhook timeout below the function's runtime limit (currently 170 seconds within 180 seconds). An unfinished handler must fail instead of acknowledging success. Preserve pending Telegram updates during webhook configuration; use `max_connections: 1` for this personal deployment and retain source-ID idempotency for retries.
