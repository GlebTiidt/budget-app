...
A single Telegram message may yield multiple ordered transaction drafts, separate debt-operation drafts, and balance observations. The Telegram client presents ordinary transactions in stable groups—income, expense, then personal transfer—while preserving their relative order inside each group. It numbers the displayed transactions and debt items consistently in the preview, while an explicit balance appears only once as `Общий остаток` in the summary. The user manages the preview by replying with ordinary text; a correction request sends only that normalized preview and direct reply back through the structured parser. Every operation is still reviewed independently, debt is never ordinary income or expense, and a balance observation is never treated as a transaction.
